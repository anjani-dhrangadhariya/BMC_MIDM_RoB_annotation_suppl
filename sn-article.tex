...
Following the above explanation, the first diamond in the flowchart asks the annotators to find a sentences/text descriptions stating that the participants could not distinguish (unaware / blinded) between the intervention and placebo.
If such description is found then annotate the text description and following the connector line of decision ``Yes'', find the sentences that describe intervention and placebo administration and annotate both the descriptions – first part where intervention is described, and second part where proper participant blinding is described.
Both these administration descriptions can be found either in adjacent sentences or far apart.
Annotate these parts with the label ``2.1 No Good''.
Annotators are likely to find this information in the methods section.
If text parts describing the intervention and placebo administration is not described then, but there are phrases in title saying ``placebo-controlled'', ``single-blind'', ``double-blind'', then mark these phrases with label ``2.1 No Good''.


If the annotators did not find any information stating that the participants were unaware of the assigned intervention, they are asked to go to the part II of the placard~\ref{fig:2_1_1}.
If the annotators find that the study is ``open-label'', or found the sentence mentioning that there was no blinding, or participants were aware, or could not be blinded, or something along the lines of the sentences mentioned below, then annotate such sentences with the label ``2.1 Yes Bad''.
...
%
%
If the information about study being ``open-label'' or participants being aware is not found, but the annotator thinks that the intervention-comparator (IC) pair is incompatible or it is not possible to achieve blinding using the I/C pairs then annotate the phrases/words that gives information about I/C (names of intervention and comparator) with the label ``2.1 Probably Yes Bad''.
